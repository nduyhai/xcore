...
// is what franz-go and segmentio clients expect.
...
func runKafkaContainer(t *testing.T) (context.Context, *tckafka.KafkaContainer, string) {
	...
	kcont, err := tckafka.Run(ctx, "confluentinc/confluent-local:7.5.0")
	if err != nil {
...
func TestIntegration_FranzGo_ProduceConsume(t *testing.T) {
	if testing.Short() {
	...
	// unique topic per test run
	topic := "it-franz-" + time.Now().Format("150405.000000")
	group := "it-franz-group-" + time.Now().Format("150405.000000")

	t.Logf("[it] using bootstrap %s", bootstrap)
	// Explicitly create a topic in case the broker disables auto-creation
	t.Logf("[it] creating topic %s...", topic)
	if err := ensureTopic(ctx, bootstrap, topic, 4, 1); err != nil {
		t.Fatalf("ensureTopic: %v", err)
	}
	t.Logf("[it] topic %s ready", topic)

	// channel-based handler to capture one message
	...
	factory, err := kafkit.GetFactory(BackendFranzGO)
	if err != nil {
		t.Fatalf("factory: %v", err)
	}

	t.Logf("[it] constructing consumer and producer...")
	...
	defer cancel()
	t.Logf("[it] starting consumer goroutine...")
	go func() {
		_ = consumer.Start(cctx)
	}()
	defer func(consumer kafkit.Consumer, ctx context.Context) {
		_ = consumer.Close(ctx)
	}(consumer, context.Background())
	defer func(prod kafkit.Producer, ctx context.Context) {
		_ = prod.Close(ctx)
	}(prod, context.Background())

	// give the consumer a moment to join the group
	t.Logf("[it] waiting for consumer group join...")
	time.Sleep(5 * time.Second)

	payload := []byte("hello-franz")
	t.Logf("[it] producing test message...")
	if err := prod.SendWith(ctx, payload, kafkit.WithKey([]byte("k1"))); err != nil {
		t.Fatalf("produce: %v", err)
	}
	t.Logf("[it] produced; waiting to receive...")
	select {
	case got := <-msgCh:
		...
		t.Logf("[it] received message OK")
	case <-time.After(10 * time.Second):
		t.Fatal("did not receive message in time")
	}
}

func TestIntegration_FranzGo_HashBalancer_SameKeySamePartition(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping integration test in -short mode")
	}
	requireDocker(t)
	...
	topic := "it-franz-hash-" + time.Now().Format("150405.000000")
	group := "it-franz-hash-group-" + time.Now().Format("150405.000000")
	...
	// wait for two partitions to be collected
	var p1, p2 int
	select {
	case p1 = <-partsCh:
